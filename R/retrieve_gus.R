...
#' - farms_10_15_ha}
#'
#'@param save {TRUE ora FALSE; default = TRUE - save retrieved data into .rds file into working directory, FALSE}
#'
...
retrieve_gus <- function (topic, save = TRUE) {
  ...
  for (i in 1:nrow(variables_matrix))
    ...
    ifelse (variables_matrix$v_name[i] == topic, index <- i, 0)
        ...
        for (k in 1:91) {url <- paste0("https://bdl.stat.gov.pl/api/v1/data/by-variable/", variables_matrix[index,2], "?format=json&page-size=30&unit-parent-id=0", units_codes[[k]], "&unit-level=6")
        urls <- rbind(url, urls)
        }

              for (m in 1:91) {# Iterate over 91 elements included in the URLs list
  ...
  if (
    ...
    # Test - identify and print NUTS6 units (gmina) out of 91 selected, which data has been not retrieved
    #topic <- read_rds(filename)
