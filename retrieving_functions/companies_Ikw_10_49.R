...
for (k in 1:91) {urls <- paste0("https://bdl.stat.gov.pl/api/v1/data/by-variable/262077?format=json&page-size=30&unit-parent-id=0", codes[[k]], "&unit-level=6")
...
write_rds(df_total_result2, "Data/companies_Ikw_10_49.rds")
companies_Ikw_10_49 <- read_rds("Data/companies_Ikw_10_49.rds")

#Check names of gmina(s), that have been not retrieved
retrieved <- as.data.frame(unique(companies_Ikw_10_49$name))
names(retrieved)[1] <- c("name")
template <- as.data.frame(unique(variables_dataframe$name))
names(template)[1] <- c("name")
missing <- anti_join(template, retrieved) %>% select ('name') %>% print() 
